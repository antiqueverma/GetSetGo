...
#ifndef GETSETGO_INC_ESP_AT_H_
#define GETSETGO_INC_ESP_AT_H_
...
extern char *CMD_OUT[__AT_T_LASTCMD];
extern char *CMD_IN[__AT_R_LASTCMD];
...
uint8_t ESP_rqstDataTransfer(TaskHandle_t task, uint8_t linkId, uint8_t priority, uint8_t flags, uint16_t port, char *ipAddress,uint8_t *buffer, uint16_t size);
void ESP_Init(void);
