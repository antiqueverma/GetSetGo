...
gsg_queueObject_t 	objSER_RX_Queue;
uint8_t				bufSER_RX_Queue[SER_RX_DATA_BUFF_SIZE];
...
//		SER_RX.BUFF[SER_RX.ToDo] = 0x00;  //Reset the next location
	...
	QUE_Insert(&objSER_RX_Queue,&UART_DREG);
...
void SER_Init(void)
{
	...
	QUE_Init(&objSER_RX_Queue,&bufSER_RX_Queue,SER_RX_DATA_BUFF_SIZE,1,QUE_TYP_FIFO);
...
void SER_Reset(void)
{
    ...
    memset(bufSER_RX_Queue,0x00,sizeof(bufSER_RX_Queue));
